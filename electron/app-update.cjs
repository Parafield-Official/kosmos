...
function createAppUpdater({
  ...
  async function install() {
    ...
    if (installing) return { installed: false, reason: "An update restart is already being prepared." };
    installing = true;
    try {
      const ready = await beforeInstall();
      if (!installing) return { installed: false, reason: "The update was interrupted. Your work remains open." };
      if (ready?.ok !== true) {
        installing = false;
