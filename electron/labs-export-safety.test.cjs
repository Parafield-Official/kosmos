...
// Exercise the real export orchestration and filesystem. Codec work is stubbed
// so a failed publication can be injected deterministically on every platform.
function loadExport(failPublish) {
  const filename = path.join(__dirname, 'labs-audio.cjs');
  const localRequire = createRequire(filename);
  const wrappedFs = { ...fs, rename: async (from, to) => {
  ...
  const helperContext = { require: name => name === 'node:fs/promises' ? wrappedFs : localRequire(name), module: { exports: {} }, process };
  vm.runInNewContext(fsSync.readFileSync(path.join(__dirname, 'file-utils.cjs'), 'utf8'), helperContext);
  const context = {
    require: name => name === 'electron' ? { app: {}, shell: { showItemInFolder() {} } }
      : name === 'node:fs/promises' ? wrappedFs
      : name === './file-utils.cjs' ? helperContext.module.exports : localRequire(name),
    module: { exports: {} }, __dirname, process, Buffer, console,
  };
  vm.runInNewContext(fsSync.readFileSync(filename, 'utf8') + `
    ...
    loadCoreModule = name => name === 'master' ? {
      ...
      deliveryProfile: () => ({ sampleRate: 44100, folderName: 'acx', extension: 'mp3' }),
      measurePcm: () => ({ traffic_light: 'green' }),
    } : name === 'export' ? {
      chapterFileName: () => '01_chapter.mp3', buildExportPlan: () => ({ readmeFiles: [] }),
      reportText: () => 'report', revealTargetInExportPack: files => files[0],
    } : {};
  `, context, { filename });
...
describe('Labs export preservation', () => {
  it('preserves the previous export and all project assets when publication fails', async () => {
    ...
    try {
      ...
      for (const dir of ['audio', 'manuscript', 'export/acx-handoff']) await fs.mkdir(path.join(root, dir), { recursive: true });
      ...
      const result = await loadExport(true)({ folder: root, mode: 'handoff', chapters: [{ id: 'one', workingFile: 'take.wav' }] });
