...
function createShutdownGuard({ askRenderer, releaseRenderer }) {
  ...
  async function run(operation) {
    if (closing) {
      throw new Error("Kosmos is closing. Please wait before starting more work.");
    }
    pending += 1;
    try {
  ...
  function prepare() {
    ...
    if (pending) {
      return Promise.resolve({ ok: false, reason: "Kosmos is still saving or processing audio. Wait for it to finish, then try again." });
    }
    closing = true;
    preparation = (async () => {
      try {
...
function requestRendererApproval({ win, ipcMain, isTrusted, timeoutMs = 5000 }) {
  ...
  return new Promise((resolve) => {
    ...
    const listener = (event, replyToken, result) => {
      if (event.sender === sender && replyToken === token && isTrusted(event)) {
        finish(result);
      }
    };
    const timer = setTimeout(() => finish({
      ok: false,
      reason: "Kosmos could not confirm that your work is saved. Try again when the window responds.",
    }), timeoutMs);
    ipcMain.on("labs:shutdown-reply", listener);
    try {
      sender.send("labs:shutdown-request", token);
    } catch {
      finish({ ok: false, reason: "The window is unavailable. Kosmos will stay open." });
    }
  });
}
