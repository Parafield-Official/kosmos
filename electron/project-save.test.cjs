...
describe("recoverable project metadata", () => {
  ...
  let file;
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), "kosmos-save-"));
  ...
  it("serializes overlapping revisions and keeps the previous valid revision", async () => {
    ...
    await Promise.all(Array.from({ length: 12 }, (_, index) => saveProjectJson(file, { chapters: [], revision: index + 1 })));
  ...
  it("preserves the last project on failed publication and permits a later retry", async () => {
    ...
    const spy = vi.spyOn(fs, "rename").mockImplementation(async (from, to) => {
      if (to === file) {
        throw Object.assign(Error("disk unavailable"), { code: "EIO" });
      }
  ...
  it("recovers a truncated marker without overwriting its valid backup", async () => {
    await saveProjectJson(file, { chapters: [], revision: 1 });
