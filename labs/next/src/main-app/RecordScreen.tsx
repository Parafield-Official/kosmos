...
export function RecordScreen({
  ...
  async function startRecording() {
    ...
    try {
      ...
      cleanupCapture();
      if (!pcm16kCountRef.current && !retainedTake.current.hasPending) {
  ...
  async function stopAndSave() {
    ...
    try {
      ...
      // transcription. Keep recordingRef true just long enough for its result.
      ...
      const file = await retainedTake.current.run(async () => {
        const existing = current?.originalFile
          ...
          : null;
        if (current?.originalFile && !existing) {
          throw new Error("The previous take could not be read. Retry when the project folder is available.");
        }
        ...
        if (!savedFile) {
          throw new Error("The recording could not be saved. Your take is still in memory; keep Kosmos open and retry.");
        }
        return savedFile;
      });
      pcm16kRef.current = [];
      ...
      onChange(applyOriginalTape(projectRef.current, chapterId, {
        file,
      ...
      window.kosmosNext?.setUnsavedWork?.(recordingKey, null);
      setFollowHint(recordedPct >= 0.98
